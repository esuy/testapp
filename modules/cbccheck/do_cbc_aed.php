...
if($_SERVER['CONTENT_LENGTH'] > 0){
	...
	if(count($entries) > 0){
		...
		$dbe->cbc_village = mysql_real_escape_string($_POST['cbc_village']);
		$dbe->cbc_name = mysql_real_escape_string($_POST['cbc_name']);
		$dbe->cbc_center_id = (int)$_POST['cbc_clinic_id'];
		$dbe->cbc_location = mysql_real_escape_string($_POST['cbc_location']);

		foreach ($entries as $key => $evals){
			$dbe1 = clone $dbe;
			$entry_date = new CDate ( $evals ["entry_date"] );
			$dbe1->cbc_entry_date = $entry_date->format ( FMT_DATE_MYSQL );
			$dbe1->cbc_adm_no = mysql_real_escape_string($evals['cbc_adm_no']);
			$dbe1->cbc_client_id = mysql_real_escape_string($evals['cbc_client_id']);
			...
			$dbe1->cbc_remarks = mysql_real_escape_string($evals['cbc_remarks']);
